    ...
    <Box p={3}>
      ...
      <Box display="flex" alignItems="center" mb={3}>
        ...
        <Box>
          ...
          <Typography variant="body2">Status: {doctor.availabilityStatus}</Typography>
        </Box>
      </Box>
            ...
            <TableBody>
              {patients.map((patient) => (
                <TableRow key={patient.id}>
                  ...
                  <TableCell>
                    <Button onClick={() => router.push(`/patients/${patient.recordId}`)}>
                      View Record
                    </Button>
                  </TableCell>
                </TableRow>
              ...
              <TableBody>
                {prescriptions.map((prescription) => (
                  <TableRow key={prescription.id}>
                    <TableCell>{patients.find((p) => p.id === prescription.patientId)?.name || 'Unknown'}</TableCell>
              ...
              <TableBody>
                {caseNotes.map((note) => (
                  <TableRow key={note.id}>
                    <TableCell>{patients.find((p) => p.id === note.patientId)?.name || 'Unknown'}</TableCell>
              ...
              <TableBody>
                {diagnosticOrders.map((order) => (
                  <TableRow key={order.id}>
                    <TableCell>{patients.find((p) => p.id === order.patientId)?.name || 'Unknown'}</TableCell>
      ...
      {tabValue === 7 && (
        <Box>
          ...
          <Typography>Patients Seen Today: {performance.patientsToday || 0}</Typography>
          <Typography>Admissions: {performance.admissions || 0}</Typography>
          <Typography>Average Consultation Time: {performance.avgConsultTime || 0} minutes</Typography>
        </Box>
      )}
    </Box>
  );
